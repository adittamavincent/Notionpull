...
- **Granular Database Exports:** Configure which columns to export for each database, complete with a live data preview. Database views are automatically detected from URLs to perfectly respect your configured property ordering, column visibility, and widths. Full column details (ordering, widths, and visibility) are perfectly preserved during database configurations and fully exported ready. Exported database headers merge properties and column layouts into a clean, unified metadata block (preventing redundant column names) while including the standard Database ID and View ID.
- **Smart Entry conditioning:** Databases with exactly one selected row are elegantly exported as a vertical property list under the entry title instead of a wide, empty-looking table structure, ensuring ultimate readability.
...
- **Interactive Copy & Feedback Buttons:** Copy buttons inside both the API Tracking modal and the main Export dialog are engineered with instant, high-satisfaction visual state feedback. Clicking any copy button instantly transitions the button styling, showing a premium checkmark indicator and a clear "Copied!" confirmation.
...
- **Relation and Rollup Transparency:** Column types such as 'relation' and 'rollup' explicitly display the reference title. Includes a premium togglable **Relation IDs** setting to seamlessly append the underlying Notion ID for precise relational mapping.
